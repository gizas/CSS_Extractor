...
class test1(unittest.TestCase):
    def setUp(self):
        FORMAT = "%(message)-s"
        logging.basicConfig(format=FORMAT,filename='reportExtAllCSS.txt', filemode='a', level=logging.INFO)
        self.logger2 = logging.getLogger("f2")#Create second logger
        self.handler2 = logging.FileHandler('reportExtAllCSS.txt',mode='a')#Create Filehandler
    ...
    def test_2(self):
        ...
        logging.warning("Sites toolbar not appeared: %d, percentage: %d %%", fail_counter, percentage_fail)
...
if __name__ == "__main__":
    log_file = "reportExtAllCSS.txt"
    f = open(log_file, "a")
    #Read testing urls from file urls.txt
    lines = open("reportExtCSSmodified.txt").read().splitlines()
